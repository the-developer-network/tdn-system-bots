/**
 * @module bots/vue/news
 * @description Vue.js news bot.
 * Monitors the Vue.js blog RSS feed and publishes
...
/** @type {string} Vue.js Blog RSS feed URL */
const VUE_RSS_URL = "https://blog.vuejs.org/feed.rss";

/** @type {import('../../api/client.js').TdnClient} API client for the Vue bot */
const client = createTdnClient("vue");

/** @type {BaseNewsWatcher} Vue.js news watcher (30-minute interval) */
...
watcher.on("new_article", async (news) => {
    log("Vue-News-Bot", "INFO", `Preparing payload for: ${news.title}`);
...
/**
 * Starts the Vue.js news bot.
 * Polls the RSS feed every 30 minutes.
 */
export function startVueNewsBot() {
    watcher.start();
}
