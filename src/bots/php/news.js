/**
 * @module bots/php/news
 * @description PHP news bot.
 * Monitors the PHP.net news RSS feed and publishes
 * a post to the TDN platform when a new article is detected.
 */

import { log } from "../../logger.js";
import { createTdnClient } from "../../api/client.js";
import { BaseNewsWatcher } from "../../core/BaseNewsWatcher.js";
import { buildNewsPost } from "../../core/utils.js";
import path from "path";
import { fileURLToPath } from "url";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

/** @type {string} PHP.net news RSS feed URL */
const PHP_RSS_URL = "https://www.php.net/feeds/news.xml";

/** @type {import('../../api/client.js').TdnClient} API client for the PHP bot */
const client = createTdnClient("php");

/** @type {BaseNewsWatcher} PHP news watcher (30-minute interval) */
const watcher = new BaseNewsWatcher({
    name: "PHP-News",
    rssUrl: PHP_RSS_URL,
    stateDir: __dirname,
    intervalMinutes: 30,
});

/**
 * Handles new article events by building and publishing a post to TDN.
 * @param {object} news - RSS article data
 */
watcher.on("new_article", async (news) => {
    log("PHP-News-Bot", "INFO", `Preparing payload for: ${news.title}`);

    const payload = buildNewsPost({
        title: news.title,
        description: news.description,
        link: news.link,
        tag: "#php",
        thumbnail: news.thumbnail,
    });

    await client.createPost(payload);
});

/**
 * Starts the PHP news bot.
 * Polls the RSS feed every 30 minutes.
 */
export function startPHPNewsBot() {
    watcher.start();
}
